...
useCols = arange(4,1558)

# load the training data
data = numpy.loadtxt(open(dataSetName+".data","rb"),delimiter=",",usecols=useCols)

print data.shape

useCols2 = arange(1558,1559)

data_target_string = numpy.loadtxt(open(dataSetName+".data","rb"),dtype=str,delimiter=",",usecols=useCols2)

data_target = zeros(data_target_string.shape[0], dtype=int)

for i in arange(data_target_string.shape[0]):
	if data_target_string[i] == 'ad.':
		data_target[i] = 1
	else:
		data_target[i] = 0
...
data_all = concatenate((data, data_target.T), axis=1)

print data_all.shape

# get the dimensions of the trainging data matrix
mall,nall = data.shape
...
for position in arange(numCombinations):
	...
	X_train = data_all[:1500,:data_all.shape[1]-1]
	y_train = data_all[:1500,data_all.shape[1]-1]

	X_test = data[1500:,:data_all.shape[1]-1]
	y_test = data_all[1500:,data_all.shape[1]-1]
