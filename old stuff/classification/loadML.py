...
data_load = numpy.loadtxt(open("movielens/ratings.dat","rb"),dtype=int,delimiter="::")

rateData = zeros((6040, 3952),dtype=int)
...
for row in data_load:
	rating = row[2]
	if rating >= 4:
		rateData[row[0]-1][row[1]-1] = 1
	elif rating <=3:
		rateData[row[0]-1][row[1]-1] = 0
	else:
		rateData[row[0]-1][row[1]-1] = -1
...
for a in arange(numCombinations):
	...
	# current movie we are running the test for
	movieID = a

	rateDataTrunc = copy(rateData[rateData[:,movieID]>=0,:])

	print rateDataTrunc.shape

	#split matrix into test and train data
	data_train_raw = rateDataTrunc[:5000]

	y_train = data_train_raw[:,movieID]
	X_train = delete(data_train_raw, movieID, 1)

	data_test_raw = rateDataTrunc[5000:5100]

	y_test = data_test_raw[:,movieID]
	X_test = delete(data_test_raw, movieID, 1)
	...
	posSize = data_train[y_train==1].shape[0]
	negSize = data_train[y_train==0].shape[0]
	
	#if posSize < negSize:
	#	dataSet[0] = data_train[y_train==0][:posSize]
	#	dataSet[1] = data_train[y_train==1]
	#else:
	#	dataSet[1] = data_train[y_train==1][:negSize]
	#	dataSet[0] = data_train[y_train==0]
