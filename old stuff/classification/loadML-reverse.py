...
rateData = zeros((3952, 6040),dtype=int)
...
for row in data_load:
	rating = row[2]
	if rating >= 4:
		rateData[row[1]-1][row[0]-1] = 1
	elif rating <=3:
		rateData[row[1]-1][row[0]-1] = 0
	else:
		rateData[row[1]-1][row[0]-1] = -1
...
for a in arange(numCombinations):
	...
	rateDataTrunc = copy(rateData[rateData[:,userID]>=0,:])

	print rateDataTrunc.shape

	#split matrix into test and train data
	data_train_raw = rateDataTrunc[:3000]

	y_train = data_train_raw[:,userID]
	X_train = delete(data_train_raw, userID, 1)

	data_test_raw = rateDataTrunc[3000:3100]

	y_test = data_test_raw[:,userID]
	X_test = delete(data_test_raw, userID, 1)
