...
for position in arange(n):
	...
	for i in arange(n-1):
		...
		assign[:i] = 1
		comp0 = append(comp0, MComponent(data0[:,target==1], assign))
		comp1 = append(comp1, MComponent(data1[:,target==1], assign))
	...
	for i in arange(n-1):
		for j in arange(n-1):
			...
			for k in arange(mv):
				...
				pr0 = comp0[i].prob(data_valid[k,target==1])
				pr1 = comp1[j].prob(data_valid[k,target==1])

				pr0 = pr0 * (1.0-prior[position])
				pr1 = pr1 * prior[position]

				if data_valid[k][position]==0:
