...
class IndComponent:
	...
	def __init__(self, data):
		...
		for j in arange(len(sortedMsTemp)):
			#print j
			if abs(previousValue-sortedMsTemp[j]) <= 0.01:
		...
		for c in arange(1,n):
			...
			for i in self.splitNumbers:
				...
				for j in arange(c,len(sortedCovTemp)):
					#print j
					if abs(previousValue-sortedCovTemp[j]) >= 0.1:
...
for dataSetName in dataSetList:
	...
	# append training and validation data -- we don't use the valdiation data for anything
	#append(data, data_valid)
