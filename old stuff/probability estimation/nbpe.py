...
# get the dimensions of the training data matrix
m,n = data.shape

# get the dimensions of the validation data matrix
...
for c in arange(0,n):
	...
	data_valid0 = data_valid[data_valid[:,position]==0]

	# get the matrix where the position column's value is 1
	data1 = data[data[:,position]==1]
	data_valid1 = data_valid[data_valid[:,position]==1]
	...
	# make a copy of the data where the current target variable is 0
	dataTemp = copy(data0[:,target==1])
	dataTempValid = copy(data_valid0[:,target==1])

	#### Compute a partition greedily ##############

	assign = zeros(n-1,dtype=int)
	maxAssign = zeros(n-1, dtype=int)
	comp = MComponent(dataTemp, maxAssign)
	maxLL = comp.ll(dataTemp)
		...
		for k in arange(n-1):
			...
			for j in arange(n-1):
				if assign[j] != m-1:
					...
					tassign[j] = m-1
					print tassign
					comp = MComponent(dataTemp, tassign)
					currLL = comp.ll(dataTempValid)
					print currLL
					if currLL > maxLL:
						maxLL = currLL
						maxJ = j
	...
	comp0 = MComponent(dataTemp, maxAssign)

	#####

	# make a copy of the data where the current target variable is 0
	dataTemp = copy(data1[:,target==1])
	dataTempValid = copy(data_valid1[:,target==1])
		...
		for k in arange(n-1):
			...
			for j in arange(n-1):
				if assign[j] != m-1:
					...
					print tassign
					comp = MComponent(dataTemp, tassign)
					currLL = comp.ll(dataTempValid) + log(prior[position])
					print currLL
					if currLL > maxLL:
						maxLL = currLL
						maxJ = j
	...
	comp1 = MComponent(dataTemp, maxAssign)
