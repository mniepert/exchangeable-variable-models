...
y_train_all = mnist.target[:60000]
y_test_all = mnist.target[60000:70000]

# and the data
X_train_all = X_both[:60000]
X_test_all =  X_both[60000:70000]

MNISTCombinations = list(combinations(unique(y_train_all), 2))
numCombinations = len(MNISTCombinations)
...
for categoriesTuple in MNISTCombinations:
	...
	for i in arange(numOfClasses):
		dataSet[i] = data_train[y_train==categories[i]]
	...
	for i in arange(mt):
		...
		for j in arange(numOfClasses):
			...
			pr[j] = pr[j] + log(float(sum(y_train==categories[j])) / float(len(y_train)))

		# check whether prediction is correct
		if y_test[i]==categories[argmax(pr)]:
			correctCounter += 1
